<template>
  <form @submit.prevent="handleSubmit" class="space-y-4">
    <div>
      ...
      <p
        v-if="(touched.category_id || hasAttemptedSubmit) && errors.category_id"
...
interface Emits {
  (e: "submit", data: SubcategoryFormData): void;
  (e: "cancel"): void;
}
...
const errors = ref<Partial<Record<keyof SubcategoryFormData, string>>>({});
const touched = ref<Partial<Record<keyof SubcategoryFormData, boolean>>>({});
...
const validateForm = () => {
  try {
    subcategorySchema.parse(formData.value);
    errors.value = {};
    return true;
  } catch (error: any) {
    ...
    if (error.errors) {
      error.errors.forEach((err: any) => {
        if (err.path && err.path[0]) {
          newErrors[err.path[0] as keyof SubcategoryFormData] = err.message;
        }
      });
    }
...
watch(
  ...
  (newData) => {
    if (!newData) {
      formData.value = { name: "", category_id: "" };
      errors.value = {};
      touched.value = {};
